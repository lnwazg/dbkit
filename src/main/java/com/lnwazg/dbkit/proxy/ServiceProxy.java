...
import com.lnwazg.dbkit.utils.DsManager;
...
public class ServiceProxy
{
    ...
    /**
     * 以自适应数据源的方式代理Service类，使用可变数据源对象<br>
     * 当某个service方法被@Transactional标注后，代表其要被自动事务控制<br>
     * 此时，需要确定档期事务管理的数据源信息，规则如下：
     * 1.若该注解的dsName不为空，则采用该dsName指向的数据源
     * 2.否则，检查该Service的成员字段，若继承自MyJdbc的成员字段数量刚好为1，那么采用该字段的DAO所指向的数据源;
     *         否则，若继承自MyJdbc的成员字段数量超过1，则报错：未指定明确的数据源
     * @author nan.li
     * @param clazz
     * @return
     */
    @SuppressWarnings("unchecked")
    public static <T> T proxyService(Class<T> targetClass)
    {
        ...
        MethodInterceptor methodInterceptor = new MethodInterceptor()
        {
                ...
                throws Throwable
            {
                ...
                if (method.isAnnotationPresent(Transactional.class))
                {
                    ...
                    String dsName = transactional.dsName();
                    if (StringUtils.isNotEmpty(dsName))
                    {
                        jdbc = DsManager.getDataSourceByName(dsName);
                    }
                    else
                    {
                        //查找字段
                        Field[] fields = targetClass.getDeclaredFields();
                        List<Field> matchedFields = new ArrayList<>();//MyJdbc对象字段列表
                        for (Field field : fields)
                        {
                            if (MyJdbc.class.isAssignableFrom(field.getType()))
                            {
                        ...
                        {
                            Logs.e("Service类" + targetClass.getSimpleName() + "中的MyJdbc成员数量超过1个，"
                                + "因此无法唯一确定service方法" + method.getName() + "自动事务管理的数据源，请手动指定@Transactional的dsName参数！");
                            return null;
                        }
                        else if (matchedFields.size() == 0)
                        {
                            Logs.w("Service类" + targetClass.getSimpleName() + "无MyJdbc成员，因此service方法" +
                                method.getName() + "上虽然声明了@Transactional，但是忽略事务管理！");
                            return proxy.invokeSuper(obj, args);
                        }
                        else
                        {
                            //可以确定该唯一的MyJdbc成员了
                            Class<?> clazz = matchedFields.get(0).getType();//获得了该成员的类
